      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ position: 'relative' }}>
          <button onClick={() => { setShowNotif(!showNotif); setShowStatus(false) }}
            style={{ position: 'relative', width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: showNotif ? 'var(--surface2)' : 'transparent', color: 'var(--text2)', border: '1px solid transparent', transition: 'all .15s' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--surface2)'}
            onMouseLeave={e => { if (!showNotif) e.currentTarget.style.background = 'transparent' }}>
            <IC name="bell" size={17} />
            {unread > 0 && <span style={{ position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: '50%', background: 'var(--danger)', border: '2px solid var(--surface)' }} />}
          </button>
          ...
          {showNotif && (
            <div className="slide-up" style={{ position: 'absolute', right: 0, top: 44, width: 360, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, boxShadow: '0 16px 48px rgba(0,0,0,0.4)', zIndex: 100, overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>Notifications {unread > 0 && <span style={{ fontSize: 11, background: 'var(--danger)', color: '#fff', borderRadius: 99, padding: '1px 7px', marginLeft: 6 }}>{unread}</span>}</span>
                {unread > 0 && <button onClick={markAllRead} style={{ fontSize: 12, color: 'var(--accent)', cursor: 'pointer' }}>Mark all read</button>}
              </div>
              ...
              <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
                <button onClick={() => { setShowNotif(false); router.push('/dashboard/notifications') }} style={{ fontSize: 12, color: 'var(--accent)', cursor: 'pointer' }}>View all notifications</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ position: 'relative' }}>
          <button onClick={() => { setShowStatus(!showStatus); setShowNotif(false) }}
            ...
            <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', display: 'none' }}>{user?.name}</span>
