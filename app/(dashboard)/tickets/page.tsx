...
export default function TicketsPage() {
  ...
  const createTicket = async () => {
    ...
    setCreating(true)
    try {
      const data = await post('/tickets', { ...newTicket, tags: newTicket.tags.split(',').map(t => t.trim()).filter(Boolean) })
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ width: 360, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--surface)', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {!loading && filteredTickets.length === 0 && <Empty icon="ticket" title="No tickets" subtitle="Adjust your filters or create a new ticket" />}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 14 }}>
                ...
                {(isAgent || isManager) && (
                  <select value={selected.status} onChange={e => updateStatus(selected.id, e.target.value)}
                    style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '7px 12px', color: 'var(--text)', fontSize: 13, flexShrink: 0, cursor: 'pointer' }}>
                    {['OPEN', 'IN_PROGRESS', 'PENDING', 'RESOLVED', 'CLOSED'].map(s => <option key={s} value={s}>{s.replace('_', ' ')}</option>)}
                  </select>
                )}
              </div>
            ...
            <Card style={{ marginBottom: 20 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16 }}>
                {[
                  ...
                  ['Response SLA', selected.slaResponse ? `${selected.slaResponse}h` : '—'],
                ].map(([label, value]) => (
                  <div key={label}>
                ...
                <div key={c.id} style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      {c.isInternal && (
                        <span style={{ fontSize: 11, background: 'rgba(245,158,11,0.12)', color: '#fbbf24', padding: '2px 8px', borderRadius: 6, border: '1px solid rgba(245,158,11,0.25)' }}>Internal Note</span>
                      )}
      ...
      {showNew && (
        <Modal title="Create New Support Ticket" onClose={() => setShowNew(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Textarea label="Description" value={newTicket.description} onChange={e => setNewTicket({ ...newTicket, description: e.target.value })} placeholder="Detailed description of the issue..." rows={5} />
