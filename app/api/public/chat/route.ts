...
import { sendChatStarted } from "@/lib/email/mailer"
  ...
  try {
    ...
    const chat = await prisma.chat.create({
      data: {
        ...
        messages: { create: [{ text: message, senderId: user.id, type: "TEXT" }] },
      },
    })
    if (agent) {
      await prisma.message.create({
        data: { chatId: chat.id, senderId: agent.id, type: "TEXT", text: `Hello ${name}! I am ${agent.name} from IMARAT IT Support. I will assist you now.` },
      })
      await prisma.notification.create({
        data: { userId: agent.id, type: "NEW_CHAT", title: "New public chat", text: `${name} started a chat: ${subject}`, chatId: chat.id },
      })
    }
    try {
      await sendChatStarted({ name, email, subject, agentName: agent?.name || null, chatStatus: chat.status })
    } catch (emailErr) {
